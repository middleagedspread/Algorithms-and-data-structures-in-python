...
def minimum_distance_squared(points):
    ...
    def min_dist_sqd_partition(points_list):
        if len(points_list) <= 1:
            return float("inf")
        elif len(points_list) == 2:
            return distance_squared(points_list[0], points_list[1])
        else:
            # recursive calls to get minimum within-half-array distance O(log n) time
            mid = len(points_list) //2
            min_left_sqd_distance = min_dist_sqd_partition(points_list[0:mid])
            # print("min_left_sqd_distance:", min_left_sqd_distance)
            min_right_sqd_distance = min_dist_sqd_partition(points_list[mid:len(points_list)-1])
        ...
        mid_line_x_coord = (points_list[mid-1][0] + points_list[mid][0]) / 2
    ...
    return min_dist_sqd_partition(sorted_by_x)
